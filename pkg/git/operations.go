...
func clone(ctx context.Context, workingDir, repoURL, repoBranch string) (path string, err error) {
	...
	if err := execGitCmd(ctx, args, gitCmdConfig{dir: workingDir}); err != nil {
		return "", errors.Wrap(err, "git clone --recurse-submodules")
	}
	return repoPath, nil
}
...
func mirror(ctx context.Context, workingDir, repoURL string) (path string, err error) {
	repoPath := workingDir
	args := []string{"clone", "--mirror", "--recurse-submodules"}
	args = append(args, repoURL, repoPath)
	if err := execGitCmd(ctx, args, gitCmdConfig{dir: workingDir}); err != nil {
		return "", errors.Wrap(err, "git clone --mirror --recurse-submodules")
	}
	return repoPath, nil
}
...
func fetch(ctx context.Context, workingDir, upstream string, refspec ...string) error {
	args := append([]string{"fetch", "--recurse-submodules", "--tags", upstream}, refspec...)
	...
	if err := execGitCmd(ctx, args, gitCmdConfig{dir: workingDir}); err != nil &&
		...
		return errors.Wrap(err, fmt.Sprintf("git fetch --recurse-submodules --tags %s %s", upstream, refspec))
	}
	return nil
}
